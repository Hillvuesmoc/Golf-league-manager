...
const formatScore = (score) => (score === null || score === undefined || score === "" ? "—" : score === 0 ? "E" : score > 0 ? `+${score}` : `${score}`);
...
function generateTeams({ poolIds, sizes, ratingLookup, pairHistory, topIds, weekCounter }) {
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    let candidate = sizes.map((size) => {
      ...
      cursor += size;
      return { id: uid(), playerIds, locked: false, score: null };
    });
    for (let s = 0; s < 25; s++) {
      ...
      const history = shouldArchive
        ? [
            ...s.history,
            {
              ...
              teams: prevDay.teams.map((t) => ({ playerIds: t.playerIds, score: t.score ?? null })),
            },
          ]
        : s.history;
